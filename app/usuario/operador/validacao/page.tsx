...
import VisitorModal from "@/components/operador/visitor-modal" // Importamos o modal
...
export default function ValidationPage() {
    // 4. Pegamos os dados e funções dos "cérebros"
    const { currentUser } = useAuth()
    const { accessRecords, addAccessRecord, addNotification } = useOperador()

    // 5. Estados locais da página (copiados de validation-screen.tsx)
    ...
    const [isVisitorModalOpen, setIsVisitorModalOpen] = useState(false) // Controla o modal

    useEffect(() => {
        inputRef.current?.focus()
    }, [])

    // 6. Funções locais (copiadas de validation-screen.tsx e adaptadas)
    const getAllOperators = () => {
        const visitors = accessRecords // Use 'accessRecords' do contexto
    ...
    const handleSearch = () => {
        ...
        if (found) {
            ...
            if (found.status === "blocked") {
                Swal.fire({ icon: "error", title: "Acesso Negado", text: found.blockReason || "Este veículo está bloqueado!" })
            } else {
                Swal.fire({ title: "Verificado!", icon: "success", draggable: true })
            }
        } else {
            setSelectedOperator(null)
            setSearchState("notfound")
            Swal.fire({ icon: "error", title: "Oops...", text: "Veículo não cadastrado!" })
        }
        setShowSuggestions(false)
    }
    ...
    const handleAccessAction = () => {
        ...
        if (selectedOperator.status === "blocked") {
            Swal.fire({ icon: "error", title: "Acesso Negado", text: selectedOperator.blockReason || "Este veículo está bloqueado!" })
            addNotification(`${selectedOperator.name} - Usuário Bloqueado`) // Função do cérebro
            return
        }
        ...
        const newRecord: AccessRecord = {
            ...
            status: "authorized",
            entryType: getNextEntryType(selectedOperator.plate),
        }

        addAccessRecord(newRecord) // Função do cérebro
        addNotification(`${selectedOperator.name} - Usuário Autorizado`) // Função do cérebro
    ...
    // 7. JSX (Copiado de validation-screen.tsx)
    // O <Header> já está no layout, então não o colocamos aqui.
    return (
        <div className="flex-1 flex items-center justify-center p-8 bg-white">
            <div className="w-full max-w-2xl bg-white border border-gray-200 rounded-2xl shadow-lg p-12">
                ...
                <div className="flex justify-end">
                    {searchState === "found" && selectedOperator?.status !== "blocked" ? (
                        ...
                        <button
                            onClick={() => setIsVisitorModalOpen(true)} // Apenas abre o modal
            ...
            {/* 8. O MODAL AGORA É SIMPLES
              Ele só precisa saber se está aberto, como fechar, e a placa inicial
            */}
            <VisitorModal
